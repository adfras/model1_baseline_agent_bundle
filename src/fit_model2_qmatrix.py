...
DEFAULT_CONFIG_PATH = Path("config/phase1_multikc_qmatrix_model2_fit.json")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fit explicit Q-matrix Model 2 with PyMC.")
...
def main() -> int:
    ...
    model = build_model2_qmatrix(train_data, context)
    ...
    save_posterior_npz(idata, posterior_draws_path, context, model_kind="model2")
    ...
    fit_summary = {
        "model_kind": "explicit_qmatrix_model2",
        "inference_method": str(config["inference_method"]),
        "random_seed": int(config["random_seed"]),
