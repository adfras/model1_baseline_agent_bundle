...
DEFAULT_CONFIG_PATH = Path("config/phase1_multikc_qmatrix_model3_fit.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fit explicit Q-matrix Model 3 with PyMC.")
...
def main() -> int:
    ...
    model = build_model3_qmatrix(train_data, context)
    ...
    fit_summary = {
        "model_kind": "explicit_qmatrix_model3",
        "inference_method": str(config["inference_method"]),
        "random_seed": int(config["random_seed"]),
        "state_bin_width": state_bin_width,
